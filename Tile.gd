...
# ─────────────────────────────────────────────
# SINK
# The root node (self) moves down so that mesh,
# outline, collision, and labels all sink together.
# ─────────────────────────────────────────────

var _sink_tween : Tween = null
var _surface_y  : float = 0.0   # root local Y captured in _ready
...
# Fast path: only calls _ensure_material on the very first use (when mat is null).
...
func _ready() -> void:
	...
	# Capture root Y now — this is what _sink() tweens away from.
	# Tile children (mesh, outline, collision) stay at their local positions;
	# moving the root moves everything together.
...
func _process(delta: float) -> void:
	...
	if _should_pulse and grid_manager and grid_manager.has_method("get_pulse_time"):
		var global_pulse : float = grid_manager.get_pulse_time()
		var new_pulse    : float = EMISSION_BASE + sin(global_pulse) * EMISSION_PULSE
		if new_pulse != _pulse_cache:
			_pulse_cache = new_pulse
			_safe_set_shader_param("emission_energy", new_pulse)

# ─────────────────────────────────────────────
# HOLD  (input driven by Player.gd raycast)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# REVEAL  (animated — player-triggered)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# REVEAL  (no animation — initial load / chunk reload)
# ─────────────────────────────────────────────
...
func set_revealed_no_animation(color: Color, text: String = "") -> void:
	...
	if text != "":
		...
		if grid_manager and grid_manager.has_method("get_pulse_time"):
			var gp : float = grid_manager.get_pulse_time()
			_pulse_cache = EMISSION_BASE + sin(gp) * EMISSION_PULSE
...
# ─────────────────────────────────────────────
# SINK
# Tweens the root node (self) downward so that
# mesh, outline, collision shape, and label all
# move together. Player stands on the collision
# shape which is now correctly below them.
# ─────────────────────────────────────────────
...
func _sink() -> void:
	...
	_sink_tween.tween_property(self, "position:y", _surface_y - SINK_DEPTH, SINK_DURATION)

func _set_sunk_position_immediate() -> void:
	position.y = _surface_y - SINK_DEPTH

# ─────────────────────────────────────────────
# LABELS
# Labels are children of the root node so they
# automatically ride the sink tween — no separate
# label tween is needed.
# ─────────────────────────────────────────────
...
func _bake_label(text: String, tile_color: Color) -> void:
	...
	l.position         = Vector3(0.0, 0.01, 0.0)   # local to root; rides down with it
	add_child(l)
...
func _bake_label_no_animation(text: String, tile_color: Color) -> void:
	...
	l.billboard        = BaseMaterial3D.BILLBOARD_DISABLED
	l.rotation_degrees = Vector3(-90.0, 0.0, 0.0)
	l.position         = Vector3(0.0, 0.01, 0.0)   # local to root
	add_child(l)
