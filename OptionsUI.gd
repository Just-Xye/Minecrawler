# OptionsUI.gd (Fully Styled with Click Sounds)
extends CanvasLayer
...
@onready var bodycam_check: CheckBox = $Panel/TabContainer/Graphics/BodycamCheck
@onready var bodycam_strength_slider: HSlider = $Panel/TabContainer/Graphics/BodycamStrengthSlider
...
func _play_click_sound() -> void:
	...
	if _click_player and is_instance_valid(_click_player):
		_click_player.play()
...
# ─────────────────────────────────────────────
# BUTTON HANDLERS (DIRECT, NO WRAPPERS)
# ─────────────────────────────────────────────
...
func _style_checkboxes() -> void:
	var checkboxes = [fullscreen_check, vsync_check, mute_check, invert_y_check, bodycam_check]
...
func _style_sliders() -> void:
	var sliders = [master_slider, music_slider, sfx_slider, ui_slider, sensitivity_slider, bodycam_strength_slider]
...
func _connect_signals() -> void:
	...
	if apply_button and not apply_button.pressed.is_connected(_on_apply_button_pressed):
		apply_button.pressed.connect(_on_apply_button_pressed)
	if reset_button and not reset_button.pressed.is_connected(_on_reset_button_pressed):
		reset_button.pressed.connect(_on_reset_button_pressed)
	if test_sound_button and not test_sound_button.pressed.is_connected(_on_test_sound_button_pressed):
	...
	if bodycam_check:
		bodycam_check.toggled.connect(_on_bodycam_toggled)
	if bodycam_strength_slider:
		bodycam_strength_slider.value_changed.connect(_on_bodycam_strength_changed)
...
func _load_settings() -> void:
	if _config.load(CONFIG_PATH) == OK:
		settings["fullscreen"] = _config.get_value("graphics", "fullscreen", true)
		settings["vsync"] = _config.get_value("graphics", "vsync", true)
		settings["resolution_index"] = _config.get_value("graphics", "resolution_index", 3)
		settings["quality_index"] = _config.get_value("graphics", "quality_index", 2)
		settings["shadow_index"] = _config.get_value("graphics", "shadow_index", 2)
		settings["aa_index"] = _config.get_value("graphics", "aa_index", 3)
		
		settings["master_volume"] = _config.get_value("audio", "master_volume", 0.8)
		settings["music_volume"] = _config.get_value("audio", "music_volume", 0.7)
		settings["sfx_volume"] = _config.get_value("audio", "sfx_volume", 0.8)
		settings["ui_volume"] = _config.get_value("audio", "ui_volume", 0.9)
		settings["muted"] = _config.get_value("audio", "muted", false)
		
		settings["bodycam_enabled"] = _config.get_value("graphics", "bodycam_enabled", false)
		settings["bodycam_strength"] = _config.get_value("graphics", "bodycam_strength", 0.35)
		
		settings["sensitivity"] = _config.get_value("gameplay", "sensitivity", 1.0)
		settings["invert_y"] = _config.get_value("gameplay", "invert_y", false)
		settings["language_index"] = _config.get_value("gameplay", "language_index", 0)
	else:
		_reset_to_defaults()

func _save_settings() -> void:
	_config.set_value("graphics", "fullscreen", settings["fullscreen"])
	_config.set_value("graphics", "vsync", settings["vsync"])
	_config.set_value("graphics", "resolution_index", settings["resolution_index"])
	_config.set_value("graphics", "quality_index", settings["quality_index"])
	_config.set_value("graphics", "shadow_index", settings["shadow_index"])
	_config.set_value("graphics", "aa_index", settings["aa_index"])
	_config.set_value("graphics", "bodycam_enabled", settings["bodycam_enabled"])
	_config.set_value("graphics", "bodycam_strength", settings["bodycam_strength"])
	
	_config.set_value("audio", "master_volume", settings["master_volume"])
	_config.set_value("audio", "music_volume", settings["music_volume"])
	_config.set_value("audio", "sfx_volume", settings["sfx_volume"])
	_config.set_value("audio", "ui_volume", settings["ui_volume"])
	_config.set_value("audio", "muted", settings["muted"])
	
	_config.set_value("gameplay", "sensitivity", settings["sensitivity"])
	_config.set_value("gameplay", "invert_y", settings["invert_y"])
	_config.set_value("gameplay", "language_index", settings["language_index"])
...
func _reset_to_defaults() -> void:
	...
	settings["bodycam_enabled"] = false
	settings["bodycam_strength"] = 0.35
...
func _update_ui_from_settings() -> void:
	...
	if bodycam_check:
		bodycam_check.button_pressed = settings["bodycam_enabled"]
	if bodycam_strength_slider:
		bodycam_strength_slider.value = settings["bodycam_strength"] * 100
...
func _apply_settings() -> void:
	...
	if SettingsManager:
		...
		SettingsManager.set_setting("bodycam_enabled", settings["bodycam_enabled"])
		SettingsManager.set_setting("bodycam_strength", settings["bodycam_strength"])
...
func _on_bodycam_toggled(pressed: bool) -> void:
	settings["bodycam_enabled"] = pressed
	# Apply immediately so the player can see the effect live
	var main = get_tree().current_scene
	if main and main.has_method("set_bodycam_enabled"):
		main.set_bodycam_enabled(pressed)
	_on_setting_changed()

func _on_bodycam_strength_changed(value: float) -> void:
	settings["bodycam_strength"] = value / 100.0
	var main = get_tree().current_scene
	if main and main.has_method("set_bodycam_strength"):
		main.set_bodycam_strength(value / 100.0)
	_on_setting_changed()
